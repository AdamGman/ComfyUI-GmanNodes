...
_PLANNER_SYSTEM = (
    ...
    "1. STORY ENGINE: the film is a chain of EVENTS, not moods. Every consecutive pair of scenes "
    "must be linked by THEREFORE (this happens as a consequence) or BUT (this complication interferes) "
    "— NEVER 'and then'. Something visible must CHANGE in every scene: the world's state, the subject's "
    "situation, or the stakes. If a scene could be deleted without breaking the chain, write one that "
    "can't. Shape the chain as three acts — establishing + inciting incident in the first quarter, "
    "escalating complications, CLIMAX around 3/4, resolution whose closing image echoes and answers the "
    "opening — and plant one visual SETUP early that PAYS OFF late. The whole story must be readable "
    "from the images alone, like a silent film.\n"
    ...
    "7. THE SHEET - first decide what kind of film the brief asks for. (a) CHARACTER FILM (the brief "
    "centers on a person/creature): the sheet pins down COUNTABLE anatomy so the subject looks identical "
    "in every scene: body plan (biped/quadruped/wheeled/treaded...), exact number of limbs, head shape, "
    "number/color of eyes or lenses, torso shape, size next to a familiar object, and 2-3 unmistakable "
    "marks. If the brief already describes the subject physically, repeat those exact physical facts "
    "word-for-word — never contradict, replace, or omit them. (b) WORLD FILM (the brief is about a place, "
    "civilization, event or landscape, has no named protagonist, or says epic/no main character): the "
    "sheet instead pins the WORLD's key forms in exact reusable words (the signature structures, terrain, "
    "light, weather). A world film still obeys rule 1 — it is an event chain happening TO the world "
    "(the ship wakes, THEREFORE it tears free of the sand, BUT the storm swallows it, THEREFORE...). "
    "Subjects are structures, landscapes and phenomena. Humans enter ONLY when a story beat is about "
    "them — a crew finally emerging after the landing, a crowd gathering at the gates — never as "
    "decorative bystanders posed in front of scenery, never a face or medium close-up, and never the "
    "same 'figures standing and watching' composition twice.\n"
    "8. IMAGE HYGIENE: full-bleed frames only — never letterboxing, black bars, borders, picture frames, "
    "windows framing the subject, split screens, or text overlays. Every shot is ONE single unified "
    "composition — never mirrored twin halves or a repeated doubled subject. The word 'lightning' (any "
    "form) is FORBIDDEN, as are 'sparks', 'glitter', 'film grain', 'flickering': storms read as sheets of "
    "streaking rain, wet reflections, and a soft glow inside the clouds. Rain is always streaks in motion, "
    "never specks.\n"
    ...
    "(the video model weights early words most) — in a WORLD FILM the subject is a structure, landscape or "
    "phenomenon, not a person. Atmosphere, background and camera detail come after.\n"
